...
class RevealUtil(object):
    ...
    def generate_initial_astNode_embedding(self, nodeContent: List[str]) -> np.array:
        ...
        #####################################################
        n_v = np.array([self.pretrain_model.wv[word] if word in self.pretrain_model.wv.key_to_index else
                np.zeros(model_args.vector_size) for word in token_seq]).mean(axis=0)
        #####################################################
    ...
    def generate_initial_training_datas(self, data: Dict) -> Tuple[int, List[Data], torch.LongTensor]:
        ...
        # edges
        cfgEdges = [json.loads(edge)[:2] if isinstance(edge, str) else edge[:2] for edge in data["cfgEdges"]]
        cdgEdges = [json.loads(edge) if isinstance(edge, str) else edge for edge in data["cdgEdges"]]
        ddgEdges = [json.loads(edge)[:2] if isinstance(edge, str) else edge[:2] for edge in data["ddgEdges"]]
        edges = cfgEdges + cdgEdges + ddgEdges
        edge_index: torch.LongTensor = torch.LongTensor(edges).t()

        # nodes
        nodes_info: List[Dict] = [json.loads(node_infos) if isinstance(node_infos, str) else node_infos for node_infos in data["nodes"]]
