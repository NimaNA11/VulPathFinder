...
from detectors.Devign.configurations import model_args, data_args, train_args
...
class TrainUtil(object):
    def __init__(self, w2v_model: Word2Vec, gnnNets: DevignModel, vul_type: str, abspath: str):
        self.w2v_model: Word2Vec = w2v_model
        self.gnnNets: DevignModel = gnnNets
        self.devign_util: DevignUtil = DevignUtil(w2v_model, gnnNets)
        self.vul_type = vul_type
        self.abspath = abspath

        self.train_positive: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "function/train_vuls.json"),
                                                    'r', encoding='utf-8'))
        self.train_negative: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "function/train_nors.json"),
                 'r', encoding='utf-8'))
        self.val_positive: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "function/val_vuls.json"),
                 'r', encoding='utf-8'))
        self.val_negative: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "function/val_nors.json"),
                 'r', encoding='utf-8'))
        self.test_positive: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "function/test_vuls.json"),
                 'r', encoding='utf-8'))
        self.test_negative: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "function/test_nors.json"),
                 'r', encoding='utf-8'))
    ...
    def save_best(self, epoch, eval_f1, is_best):
        ...
        pth_name = f"{model_args.model_dir.format(self.abspath, self.vul_type)}/{model_args.model_name}_{model_args.detector}_latest.pth"
        best_pth_name = f'{model_args.model_dir.format(self.abspath, self.vul_type)}/{model_args.model_name}_{model_args.detector}_best.pth'
        ckpt_path = os.path.join(model_args.model_dir.format(self.abspath, self.vul_type), pth_name)
        torch.save(state, ckpt_path)
        if is_best:
            shutil.copy(ckpt_path, os.path.join(model_args.model_dir.format(self.abspath, self.vul_type), best_pth_name))
    ...
    def evaluate(self, eval_dataloader):
        ...
        with torch.no_grad():
            for batch_graph_info in eval_dataloader:
                batch_data = [self.devign_util.generate_initial_graph_embedding(graph_info) for graph_info in batch_graph_info]
    ...
    def test(self):
        ...
        # checkpoint = torch.load(os.path.join(model_args.model_dir.format(self.abspath, self.vul_type), f'{model_args.model_name}_{model_args.detector}_best.pth'))
        ...
        with torch.serialization.safe_globals([numpy.core.multiarray.scalar]):
            ...
            os.path.join(model_args.model_dir.format(self.abspath, self.vul_type),
                     f'{model_args.model_name}_{model_args.detector}_best.pth'),
            weights_only=False
            )
        ...
        self.gnnNets.load_state_dict(new_state_dict)
        ######################################################

        # embedding data
        print("start embedding data=================")
        start_time = time()
        test_vul_graph_infos = [self.devign_util.generate_initial_training_datas(sample) for sample in
                               tqdm(self.test_positive, desc="generating feature for vul sample", file=sys.stdout)]
        test_nor_graph_infos = [self.devign_util.generate_initial_training_datas(sample) for sample in
        ...
        with torch.no_grad():
            for batch_graph_info in test_dataloader:
                batch_data = [self.devign_util.generate_initial_graph_embedding(graph_info) for graph_info in
                              batch_graph_info]
                batch = Batch.from_data_list(batch_data)
    ...
    def train(self):
        print("start embedding data=================")
        start_time = time()
        train_vul_graph_infos = [self.devign_util.generate_initial_training_datas(sample) for sample in
                                 tqdm(self.train_positive, desc="generating feature for train vul sample",
                                      file=sys.stdout)]
        train_nor_graph_infos = [self.devign_util.generate_initial_training_datas(sample) for sample in
                                 tqdm(self.train_negative, desc="generating feature for train normal sample",
                                      file=sys.stdout)]
        val_vul_graph_infos = [self.devign_util.generate_initial_training_datas(sample) for sample in
                                 tqdm(self.val_positive, desc="generating feature for val vul sample",
                                      file=sys.stdout)]
        val_nor_graph_infos = [self.devign_util.generate_initial_training_datas(sample) for sample in
    ...
    def choose_data(self):
        # load model
        checkpoint = torch.load(
            os.path.join(model_args.model_dir.format(self.abspath, self.vul_type), f'{model_args.model_name}_{model_args.detector}_best.pth'))
        self.gnnNets.load_state_dict(checkpoint['net'])
        exported_datas = list()
        # embedding data
        print("start embedding data=================")
        ...
        for idx in trange(len(test_vul_graph_infos), desc="selecting data", file=sys.stdout):
            ...
            data: Data = self.devign_util.generate_initial_graph_embedding(graph_info)
        ...
        json.dump(exported_datas, open(os.path.join(data_args.dataset_dir, "function/explain_devign.json"), 'w'
...
#     devign_model.to(model_args.device)
#     train_util = TrainUtil(pretrain_model, devign_model)
