...
class TrainUtil(object):
    def __init__(self, w2v_model: Word2Vec, gnnNets: DeepWuKongModel, vul_type: str, abspath: str):
        ...
        self.train_positive: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "slice/train_vuls.json"),
                 'r', encoding='utf-8'))
        self.train_negative: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "slice/train_nors.json"),
                 'r', encoding='utf-8'))
        self.val_positive: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "slice/val_vuls.json"),
                 'r', encoding='utf-8'))
        self.val_negative: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "slice/val_nors.json"),
                 'r', encoding='utf-8'))
        self.test_positive: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "slice/test_vuls.json"),
                 'r', encoding='utf-8'))
        self.test_negative: List[Dict] = json.load(
            open(os.path.join(data_args.dataset_dir.format(abspath, vul_type), "slice/test_nors.json"),
                 'r', encoding='utf-8'))

    def generate_initial_training_datas(self, data: Dict) -> Data:
        if model_args.with_sym:
        ...
        ################################
        n_vs = [np.array([self.w2v_model.wv[word] if word in self.w2v_model.wv.key_to_index else 
                np.zeros(model_args.vector_size) for word in token_seq.split(" ")]).mean(axis=0) 
    ...
    def test(self):
        ...
        test_vul_datas = [self.generate_initial_training_datas(sample) for sample in
                            tqdm(self.test_positive, desc="embedding vulnerable XFG", file=sys.stdout)]
        test_nor_datas = [self.generate_initial_training_datas(sample) for sample in
                            tqdm(self.test_negative, desc="embedding normal XFG", file=sys.stdout)]
        end_time = time()
    ...
    def train(self):
        ...
        path = os.path.join(model_args.model_dir.format(self.abspath, self.vul_type), f'{model_args.model_name}_{model_args.detector}_best.pth')
    ...
    def predict_single_data(self, data: Data):
        self.gnnNets.eval()
        result = torch.softmax(self.gnnNets(data=Batch.from_data_list([data]).to(model_args.device)),dim=1)
        return result.cpu().tolist()[0][1]
...
if __name__ == '__main__':
    ...
    train_util: TrainUtil = TrainUtil(pretrain_model, dwk_model, vul_type = "buffer overflow")